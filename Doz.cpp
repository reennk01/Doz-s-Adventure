...
void Doz::Begin()
{
	...
	fixtureData.doz = this;
	fixtureData.type = FixtureDataType::Doz;
...
void Doz::Update(float deltaTime)
{
...
void Doz::Draw(Renderer& renderer)
{
	renderer.Draw(textureToDraw, position,
		sf::Vector2f(facingLeft ? -1.0f : 1.0f, 2.0f), angle);
}

void Doz::OnBeginContact(b2Fixture* self, b2Fixture* other)
{
...
void Doz::OnEndContact(b2Fixture* self, b2Fixture* other)
{
	FixtureData* data = (FixtureData*)other->GetUserData().pointer;
